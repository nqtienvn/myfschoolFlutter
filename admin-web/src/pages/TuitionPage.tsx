...
export default function TuitionPage() {
  ...
  // Form State: Tạo học phí hàng loạt cho lớp
  ...
  useEffect(() => {
    ...
    // Đặt hạn nộp mặc định là cuối tháng hiện tại
    const date = new Date();
    const lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0).toISOString().split('T')[0];
    setDueDate(lastDay);
  }, []);

  // 2. Tự động tải danh sách hóa đơn học phí khi chọn Lớp + Học kỳ
  useEffect(() => {
  ...
  // 3. Tạo hóa đơn học phí hàng loạt cho cả lớp
  async function handleCreateClassBills() {
    ...
    if (!billName.trim()) {
      setError('Vui lòng nhập Tên khoản thu học phí.');
      return;
    }
    if (isNaN(billAmount) || billAmount <= 0) {
      setError('Số tiền học phí phải lớn hơn 0.');
      return;
    }
    if (!dueDate) {
      setError('Vui lòng chọn Hạn nộp học phí.');
      return;
    }

    try {
      // Bước 1: Lấy danh sách học sinh của lớp học
      ...
      setProgress({ active: true, current: 0, total, label: `Bắt đầu tạo hóa đơn...` });

      // Bước 2: Gọi API tạo hóa đơn tuần tự cho từng học sinh
      for (let idx = 0; idx < total; idx++) {
        ...
        setProgress(p => ({
          ...p,
          current: idx,
          label: `Đang tạo hóa đơn: ${student.name} (${idx + 1}/${total})...`
        }));
      ...
      setProgress(p => ({ ...p, current: total, label: 'Đã tạo hóa đơn thành công cho cả lớp!' }));
      setSuccessMsg(`Đã phát hành thành công ${total} hóa đơn học phí cho lớp học!`);
    ...
    <div>
      ...
      {progress.active && (
        <div className="progress-container">
          ...
          <div className="progress-text">Đã hoàn thành: {progress.current} / {progress.total} hóa đơn</div>
        </div>
      )}

      {/* Bộ lọc lớp và học kỳ */}
      <div className="form-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
        <div className="form-group">
          ...
          <span className="input-desc">Lớp học cần quản lý học phí</span>
        </div>
        ...
        <div className="form-group">
          ...
          <span className="input-desc">Áp dụng cho kỳ học</span>
        </div>
      </div>

      {/* Form tạo học phí hàng loạt cho lớp */}
      {classId && semesterId && !progress.active && (
        <div className="form-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', marginTop: '16px', background: '#fbfbfb' }}>
          <div style={{ gridColumn: 'span 4', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', marginBottom: '8px' }}>
            [ TẠO KHOẢN THU HỌC PHÍ HÀNG LOẠT CHO CẢ LỚP ]
          </div>

          <div className="form-group">
            <label>Tên khoản thu</label>
            <input 
              placeholder="VD: Học phí học kỳ 1, Tiền đồng phục..." 
              value={billName} 
              onChange={e => setBillName(e.target.value)} 
            />
            <span className="input-desc">Tên nội dung hóa đơn hiển thị lên app</span>
          </div>
          ...
          <div className="form-group">
            ...
            />
            <span className="input-desc">Số tiền nộp (Ví dụ: 3000000)</span>
          </div>
          ...
          <div className="form-group">
            ...
            />
            <span className="input-desc">Hạn cuối phụ huynh phải thanh toán</span>
          </div>

          <div className="form-group">
            <label style={{ visibility: 'hidden' }}>Thao tác</label>
            <button onClick={handleCreateClassBills} style={{ height: '38px', width: '100%' }}>Phát hành hóa đơn</button>
      ...
      {classId && semesterId && (
        <div style={{ marginTop: '32px' }}>
          <h3 style={{ fontSize: '15px', fontWeight: 700, textTransform: 'uppercase', marginBottom: '16px', borderBottom: '1px dashed #000000', paddingBottom: '8px' }}>
            Danh sách Hóa đơn lớp: {classes.find(c => c.id === +classId)?.name}
          </h3>

          {loadingList ? (
            <p style={{ fontSize: '13px', fontFamily: 'ui-monospace, monospace' }}>Đang tải danh sách hóa đơn học phí...</p>
          ) : items.length === 0 ? (
            <p style={{ fontSize: '13px', color: '#a3a3a3', fontStyle: 'italic' }}>Chưa có hóa đơn học phí nào được tạo cho lớp học này.</p>
          ) : (
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                {items.map((b: any) => (
                  <tr key={b.id}>
                    ...
                    <td style={{ fontFamily: 'ui-monospace, monospace', fontSize: '12px' }}>{b.dueDate}</td>
                    <td>
                      {b.status === 'PAID' ? (
                        <span style={{ background: '#16a34a', color: '#ffffff', padding: '2px 8px', fontSize: '11px', fontWeight: 700 }}>
                          ĐÃ THANH TOÁN
                        </span>
                      ) : b.status === 'PROCESSING' ? (
                        <span style={{ background: '#eab308', color: '#000000', padding: '2px 8px', fontSize: '11px', fontWeight: 700 }}>
                          ĐANG XỬ LÝ
                        </span>
                      ) : (
                        <span style={{ background: '#ef4444', color: '#ffffff', padding: '2px 8px', fontSize: '11px', fontWeight: 700 }}>
                          CHƯA THANH TOÁN
                        </span>
                      )}
                    </td>
                    <td>
                      <button className="danger" onClick={() => deleteItem(b.id)}>
                        Xóa
                      </button>
                    </td>
                  </tr>
