  ...
  return (
    <div className="page-stack">
      ...
      <section className="class-generation-form">
        <div className="form-group"><label>Khối lớp</label><select value={gradeLevel} onChange={e=>setGradeLevel(Number(e.target.value))}>{Array.from({length:12},(_,i)=>i+1).map(value=><option key={value} value={value}>Khối {value}</option>)}</select><small className="input-desc">Khối áp dụng cho nhóm lớp mới.</small></div>
        ...
        <div className="form-group"><label>Số lượng lớp</label><input type="number" min={1} max={50} value={count} onChange={e=>setCount(Number(e.target.value))}/><small className="input-desc">Tạo từ 1 đến tối đa 50 lớp.</small></div>
      ...
      <div className="class-list-heading"><div><h2>Danh sách lớp & giáo viên chủ nhiệm</h2><p>Chọn giáo viên chủ nhiệm ngay trên từng dòng lớp.</p></div><span>{classes.length} lớp</span></div>
