  ...
  return (
    <main className="page-stack monitoring-page" aria-label="Theo dõi học sinh và báo cáo GVCN">
      ...
      <section className="panel">
        ...
        {!loading && <div className="table-responsive"><table><thead><tr><th>Lớp</th><th>Sĩ số</th><th>Chuyên cần</th><th>GPA</th><th>Nguy cơ</th><th>Liên hệ / họp</th></tr></thead><tbody>{summaries.map((item) => <tr key={item.classId}><td><strong>{item.className}</strong><small className="table-subtext">Khối {item.gradeLevel}</small></td><td>{item.studentCount}</td><td>{item.attendanceRate}%</td><td>{item.averageGpa}</td><td>{item.openRiskCount}</td><td>{item.parentContactCount} / {item.meetingCount}</td></tr>)}</tbody></table>{summaries.length === 0 && <div className="empty-state">Chưa có lớp phù hợp phạm vi đã chọn.</div>}</div>}
      </section>
