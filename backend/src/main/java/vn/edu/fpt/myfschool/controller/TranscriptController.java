...
@RestController @RequestMapping("/api/transcripts") @RequiredArgsConstructor public class TranscriptController { private final TranscriptService service;
 @GetMapping("/me") @PreAuthorize("hasRole('STUDENT')") public ResponseEntity<ApiResponse<StudentTranscriptDto>> mine(@RequestParam Long academicYearId,@RequestParam Long semesterId){return ResponseEntity.ok(ApiResponse.success(service.getMine(academicYearId,semesterId)));}
